...
// disableDeletionProtection removes deletion protection from the given resource if set,
...
func disableDeletionProtection(t *Harness, u *unstructured.Unstructured) {
	...
	modified := false
	// 1. spec.deletionProtection (BigtableLogicalView, BigtableAuthorizedView, BigtableMaterializedView, ComputeInstance)
	...
	// 2. spec.deletionProtectionEnabled (RedisCluster, etc.)
	if val, found, _ := unstructured.NestedBool(live.Object, "spec", "deletionProtectionEnabled"); found && val {
		if err := unstructured.SetNestedField(live.Object, false, "spec", "deletionProtectionEnabled"); err == nil {
			modified = true
		}
	}
	// 3. spec.settings.deletionProtectionEnabled (SQLInstance)
	if val, found, _ := unstructured.NestedBool(live.Object, "spec", "settings", "deletionProtectionEnabled"); found && val {
		if err := unstructured.SetNestedField(live.Object, false, "spec", "settings", "deletionProtectionEnabled"); err == nil {
			modified = true
		}
	}
...
// abandonStuckResource annotates a resource with the abandon deletion policy if it fails or times out
